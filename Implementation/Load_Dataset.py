...
class Train_Set(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        feature_vector = torch.squeeze(torch.from_numpy(np.asarray(self.concat_context_frame(sample_id,frame_id))).float())
        feature_label = torch.squeeze(torch.from_numpy(np.asarray(self.train_label[sample_id][frame_id])))
