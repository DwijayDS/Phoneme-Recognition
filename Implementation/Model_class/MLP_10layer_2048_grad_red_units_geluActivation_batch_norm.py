# -*- coding: utf-8 -*-
"""
@title: Model Definition (5 Hidden Layer, Size (2048.2048,1024,1024,512,512,256,256,128,128) gelu activation )
...
class MLP(nn.Module):
    ...
    def __init__(self,Context_length):
        ...
        self.dense5_bn = nn.BatchNorm1d(hid10)
        
        self.layer11 = nn.Linear(hid10,hid11)
        
        self.layer12 = nn.Linear(hid11,hid12)
              
        self.layer13 = nn.Linear(hid12,self.n_classes)
    ...
    def forward(self,x):
        ...
        x = F.gelu(self.dense5_bn(self.layer10(x)))
        
        x = F.gelu(self.layer11(x))
        
        x = F.gelu(self.layer12(x))
              
        x = self.layer13(x)   # We dont use activation here Because we are using cross entropy loss which has inbuild log softmax operation
